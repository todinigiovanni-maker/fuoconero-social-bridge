...
Use music_title matching a unique library title exactly (case/whitespace normalized), or music_id. “Fuoconero” and “Fuoconero (rock)” never match each other. An ambiguous title is an error. Optional image_ids must refer to featured/content images belonging to the chosen post; no arbitrary asset URLs. Scene text is not generated from the whole article. It is supplied/edited by the caller. For normal automatic Reel creation, the caller MUST draft/shorten scenes so the estimated duration is at most 22 seconds and MUST keep allow_extended_duration=false. If the estimate exceeds 22 seconds, revise the scene text before submitting instead of relying on a rejected render job. Use allow_extended_duration=true only when the user explicitly requests a longer Reel. Overflow is paginated without losing words. Resource ceiling is 600 seconds and 32 MiB per output; the renderer reports an error rather than truncating.
